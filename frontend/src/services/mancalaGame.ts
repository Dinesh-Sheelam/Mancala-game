import type { Player, MancalaState } from '../types/mancala';
...
export function makeMove(state: MancalaState, pitIndex: number): GameResult {
...
export function checkGameOver(board: number[]): boolean {
  const player1PitsEmpty = board.slice(0, 6).every((seeds) => seeds === 0);
  const player2PitsEmpty = board.slice(7, 13).every((seeds) => seeds === 0);
