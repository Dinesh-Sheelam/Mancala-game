...
## Purpose
TBD - created by archiving change update-landing-page-layout. Update Purpose after archive.
## Requirements
### Requirement: Landing Page Layout
The landing page SHALL display the user profile section at the top-left position with a user profile icon above the player name, and the main action buttons (Play and How to Play) SHALL be stacked vertically and centered horizontally on the page. The profile section SHALL have z-index z-50 to ensure clickability above other page elements.
...
- **AND** the profile section has z-index z-50 to ensure it's clickable above other page elements

#### Scenario: Player name updates reflect below icon
- **WHEN** the user edits their name in the profile section
- **THEN** an input field appears below the icon with dark background (bg-slate-800/90) and white text for visibility
- **AND** the updated name is immediately displayed below the profile icon
...
The game board SHALL display with a vintage wood texture, colorful glossy marbles, and wooden number plaques. Pits SHALL be sized at 112px (w-28 h-28) and stores SHALL be sized at 100px width and 240px height, while the board container maintains its original padding (p-20).
...
- **AND** stores are sized at 100px width and 240px height
...
Local multiplayer games SHALL allow both players to play with custom names. The multiplayer name input modal SHALL use "Multiplayer Name" as the label (not "Player 2 Name") and SHALL have proper text visibility with dark background.

#### Scenario: Player 2 name is requested
- **WHEN** the user clicks "Start Multiplayer Game"
- **THEN** a modal appears with title "Enter Multiplayer Name"
- **AND** the label displays "Multiplayer Name" (not "Player 2 Name")
- **AND** the input field has dark background (bg-slate-800/90) with white text for visibility
